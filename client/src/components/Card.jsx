...
const Card = ({ perfume }) => {
    ...
    const { cart, appliedCoupon, currentCartState } = useSelector(
        (state) => state.perfume.cartData
    );

    const isWishlisted = wishlist.some((item) => item?.perfume?._id === perfume?._id);

    const isInCart = cart.some((item) => item?.perfume?._id === perfume?._id);
    ...
    return (
        <Link to={perfume ? `/perfume/${perfume._id}` : null}>
            <div
                key={perfume.id}
                className="group relative flex flex-col justify-between bg-secondary-white/40 border border-beige-light/60 rounded-2xl p-4 hover:border-beige-light transition-colors"
            >
                ...
                <div className="mt-4 space-y-1">
                    <div className="flex justify-between items-center">
                        <span className="text-[12px] font-artistic-secondary text-beige-dark capitalize tracking-wider">
                            {perfume.category || <Skeleton />}
                        </span>
                    ...
                    <div className="flex items-center justify-start gap-1">
                        <h4 className="font-primary font-semibold text-[15px] text-primary-black capitalize tracking-tight">
                            {perfume.brand || <Skeleton />}
                        </h4>
                        <p className="text-[13px] font-semibold capitalize font-primary text-secondary-black truncate">
                            {perfume?.name || <Skeleton />}
                        </p>
                    </div>

                    <div className="flex justify-start items-end pt-3 mt-2 gap-2 border-t border-beige-light/40 ">
                        <div className="font-secondary font-bold text-sm text-secondary-black">
                            Rs {perfume?.price?.toLocaleString("en-IN") || <Skeleton />}
                        </div>
                        <div className="font-primary capitalize font-bold text-sm text-gray-500">
                            {perfume?.size || <Skeleton />}
                        </div>
                        <button
                            onClick={(e) => {
                            ...
                            className={`text-xs px-3 ml-auto py-1.5 rounded-lg transition-colors font-['Roboto',sans-serif] ${
                                perfume?.inStock <= 0
                                    ? "bg-red-900  text-white cursor-not-allowed"
                                    : isInCart || isProcessing
                                      ? "bg-gray-400 cursor-not-allowed text-white"
                                      : "bg-primary-black hover:bg-green-dark text-white"
                            }`}
                        >
                            {perfume?.inStock <= 0
                                ? " * Out of stock"
