...
export default function PerfumeUpdate() {
        ...
        const fetchPerfume = async () => {
            try {
                ...
                Object.keys(response.data).forEach((key) => {
                    if (key !== "notes" && key !== "images" && key !== "concentration") {
                ...
                setValue(
                    "concentration",
                    CONCENTRATION_MAP[response.data.concentration?.toLowerCase()] ||
    ...
    const handleFileChange = (e) => {
        ...
        const availableSlots = 5 - existingImages.length - newFiles.length;

        if (availableSlots <= 0) {
            return showErrorToast("Maximum 5 images allowed");
        }

        if (files.length > availableSlots) {
            return showErrorToast(`You can add only ${availableSlots} more image(s)`);
        }
    ...
    const onSubmit = async (data) => {
        if (totalImages < 3) {
            return showErrorToast("Please ensure at least 3 images are present");
        }

        setIsSubmitting(true);

        const toastId = showLoadingToast("Updating perfume...");
        ...
        try {
            ...
            const oldPrice = discount > 0 ? Math.round(price / (1 - discount / 100)) : price + 499;
            ...
            formData.append("notes[top]", data.top);

            formData.append("notes[heart]", data.heart);

            formData.append("notes[base]", data.base);

            formData.append("existingImages", JSON.stringify(existingImages));

            formData.append("deletedPublicIds", JSON.stringify(deletedIds));
            ...
            showSuccessToast("Perfume updated successfully", toastId);

            navigate(`/perfume/${id}`);
        } catch (error) {
            showErrorToast(error?.response?.data?.message || "Failed to update perfume", toastId);
        } finally {
                ...
                >
                    ...
                    <div className="mb-6 space-y-1 text-center md:text-left shrink-0 flex flex-col md:flex-row md:items-center md:justify-between gap-2">
                        <div>
                            ...
                            <p className="text-xs sm:text-sm font-secondary text-secondary-black">
                                Modify olfactory architecture, pricing tiers, and active catalog
                                metadata.
                            </p>
                        </div>
                            ...
                            <div className="space-y-4">
                                    ...
                                    <div className="flex flex-col gap-1.5 w-full">
                                        ...
                                        <div className="grid grid-cols-3 gap-2 h-9.5">
                                            {["male", "female", "unisex"].map((genderOption) => (
                                                <label
                                                    key={genderOption}
                                                    className={`flex items-center justify-center gap-2 px-2 text-xs font-semibold bg-primary-white border ${
                                                        errors.gender
                                                            ? "border-red-600"
                                                            : "border-beige-light/80 hover:border-beige-dark"
                                                ...
                                                >
                                                    <input
                                                        ...
                                                        {...register("gender", {
                                                            required: "Gender profile is required",
                                                        })}
                                                    />
                                                    {genderOption}
                                                </label>
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <Input
                                        ...
                                        {...register("size", {
                                            required: "Fluid volume size is required",
                                        })}
                                    />
                                    ...
                                    <div className="flex flex-col gap-1.5 w-full">
                                        ...
                                        <select
                                            className={`w-full px-3 py-2.5 text-xs font-semibold bg-primary-white border ${
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="bg-secondary-white/30 p-4 rounded-xl border border-beige-light/40 space-y-3">
                                    ...
                                    <Input
                                        ...
                                        {...register("heart", {
                                            required: "Heart notes are required",
                                        })}
                                    />
                                    <Input
                                        type="text"
                                        ...
                                        {...register("base", {
                                            required: "Base notes are required",
                                        })}
                                    />
                                </div>
                                ...
                                <div className="flex flex-col gap-1.5 w-full">
                                    ...
                                    <div className="relative group border-2 border-dashed border-beige-light/80 hover:border-beige-dark bg-secondary-white/40 rounded-xl p-4 transition-all duration-300 flex flex-col items-center justify-center cursor-pointer min-h-27.5">
                                        ...
                                        <svg
                                            className="w-6 h-6 text-beige-dark/60 group-hover:text-beige-dark mb-1 transition-colors duration-300"
                                            fill="none"
                                            stroke="currentColor"
                                            strokeWidth="2"
                                            viewBox="0 0 24 24"
                                        ...
                                        <p className="text-xs font-semibold text-secondary-black text-center font-primary">
                                            <span className="text-beige-dark group-hover:text-beige-accent underline underline-offset-2 transition-colors">
                                                Upload new assets
                                            </span>{" "}
                                            to overwrite gallery
                                        </p>
                                        <p className="text-[10px] text-secondary-black/50 font-secondary mt-0.5 text-center">
                                            Leaving blank retains active images (Batch limit: 5
                                            items)
                                        </p>
                                    </div>
                    ...
                    <div className="pt-4 shrink-0 mt-4 border-t border-beige-light/30 flex items-center justify-end gap-3">
                        ...
                        <Button
                            ...
                            child={
                                isSubmitting ? "Updating Database..." : "Commit Changes to Catalog"
                            }
                            colorSchema={
                                isSubmitting
                                    ? "bg-beige-dark text-primary-white cursor-not-allowed opacity-75"
                                    : "bg-primary-black hover:bg-primary-black/90 text-primary-white shadow-sm"
                            }
