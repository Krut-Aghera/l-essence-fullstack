        ...
        return (
            <div className="min-h-screen bg-secondary-white text-primary-black font-primary flex flex-col justify-center items-center space-y-4">
                <p className="text-sm font-secondary text-secondary-black">
                    {error || "No dashboard registry found."}
                </p>
                <button
            ...
            <div className="grow w-full max-w-7xl mx-auto px-4 sm:px-6 py-10 space-y-10">
                <div className="border-b border-beige-light pb-5 flex flex-col md:flex-row md:items-end md:justify-between gap-4">
                    <div>
                        ...
                        <p className="font-secondary text-xs text-secondary-black/70 mt-1">
                            Real-time system insights, sandbox mock data, and database inventory
                            levels.
                        </p>
                    </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-5">
                    ...
                    <div className="bg-primary-white border border-beige-light p-5 rounded-2xl flex items-center gap-4 shadow-sm hover:shadow-md transition-shadow">
                        ...
                        <div>
                            <span className="text-[10px] font-secondary uppercase tracking-widest text-secondary-black/50 font-bold block">
                                Gross Revenue
                            </span>
                                    ...
                                    <tbody className="divide-y divide-beige-light/40 text-xs font-secondary text-primary-black">
                                        {recentOrders && recentOrders.length > 0 ? (
                                            recentOrders.map((order) => (
                                                <tr
                                                    key={order._id}
                                                    className="hover:bg-secondary-white/40 transition-colors"
                                                >
                                                    ...
                                                    {/* Client Identity */}
                                                    <td className="p-4">
                                                        <p className="font-semibold capitalize text-primary-black">
                                                            {order.user?.name || "Anonymous"}
                                                        </p>
                                                        <p className="text-[10px] text-secondary-black/60 normal-case font-mono">
                                                            {order.user?.email}
                                                        </p>
                                                    </td>
                                                    ...
                                                    <td className="p-4">
                                                        <span
                                                            className={`inline-block px-2.5 py-0.5 rounded-full text-[9px] uppercase tracking-wide font-bold border font-mono ${
                                            ...
                                            <tr>
                                                <td
                                                    colSpan="4"
                                                    className="p-12 text-center text-secondary-black/60"
                                                >
                                                    <FaBoxOpen className="mx-auto text-2xl mb-3 text-secondary-black/30" />
                                                    No active transactions currently recorded in the
                                                    system ledger.
                                                </td>
                                            </tr>
                                        )}
                                ...
                                {brandStats && brandStats.length > 0 ? (
                                    brandStats.map((brandInfo, index) => {
                                        ...
                                        const ratio = Math.min(
                                            (brandInfo.totalPerfumes / totalSkus) * 100,
                                            100
                                        );
                                        ...
                                        return (
                                            <div
                                                key={index}
                                                className="space-y-2 font-secondary text-xs"
                                            >
                                                <div className="flex justify-between items-center text-primary-black">
                                                    <span className="font-semibold capitalize tracking-wide text-primary-black">
                                                        {brandInfo.brand}
                                                    </span>
                                                    <span className="text-secondary-black/60 font-mono text-[11px]">
                                                        {brandInfo.totalPerfumes} items
                                                    </span>
                                                </div>
                                    ...
                                    <p className="text-xs text-secondary-black/60 text-center py-6">
                                        No brand inventory analytics parsed.
                                    </p>
                                )}
                            </div>
                        </div>
