...
const PerfumeInfoPage = () => {
    const { id } = useParams();
    if (!id)
        return <p className="text-primary-black text-2xl font-medium"> No perfume Id Found </p>;
    ...
    const isWishlisted = wishlist.some((item) => item?.perfume?._id === perfumeDetails?._id);

    const isInCart = cart.some((item) => item?.perfume?._id === perfumeDetails?._id);
    ...
    const addToWishlistHandler = async (perfumeId) => {
        try {
            ...
            showCustomToast("Added to Wishlist ❤️", "Perfume saved for later");
        } catch (error) {
    ...
    return (
        <div className="min-h-screen flex flex-col bg-secondary-white text-primary-black font-primary relative">
            ...
            <div className="max-w-7xl w-full mx-auto px-6 pt-6 text-xs tracking-wide font-secondary text-beige-dark">
                <span className="hover:text-primary-black cursor-pointer">Fragrances</span> /{" "}
                <span className="hover:text-primary-black cursor-pointer">
                    {perfumeDetails?.category}
                </span>{" "}
                / <span className="text-secondary-black font-medium">{perfumeDetails?.name}</span>
            </div>
            ...
            <main className="grow max-w-7xl w-full mx-auto px-6 py-8 grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-16 items-start">
                ...
                <div className="lg:col-span-7 space-y-6">
                    <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
                        <div>
                            ...
                            <h1 className="font-primary text-3xl capitalize md:text-4xl font-bold text-primary-black leading-tight">
                                {perfumeDetails?.name}{" "}
                                <span className="font-primary font-semibold text-secondary-black text-lg inline-block ml-1">
                                    {perfumeDetails?.concentration}
                                </span>
                            </h1>
                            <p className="font-primary text-base text-beige-dark mt-1 tracking-wide">
                                Scent Family:{" "}
                                <span className="capitalize text-secondary-black ">
                                    {formatCategory(perfumeDetails?.category)}
                                </span>
                            </p>
                        </div>
                    ...
                    <div className="flex items-center gap-2 border-y border-beige-light py-3">
                        ...
                        <span className="font-secondary text-xs text-primary-black font-semibold ml-1">
                            4.6
                        </span>
                        <span className="text-xs text-beige-dark font-secondary">
                            149 verified customer reviews
                        </span>
                    </div>
                    ...
                    <div className="space-y-1">
                        <div className="flex items-baseline gap-3">
                            <span className="text-2xl font-bold font-secondary text-primary-black">
                                Rs. {perfumeDetails?.price?.toLocaleString("en-IN")}
                            </span>
                        ...
                        <p className="text-[11px] font-secondary text-beige-dark">
                            Authentic luxury fragrances, thoughtfully curated.
                        </p>
                    </div>
                    ...
                    <div className="bg-primary-white border-2 border-beige-light/60 rounded-xl p-5 space-y-3 shadow-xs">
                        <h3 className="font-bold text-sm tracking-wide text-primary-black font-primary uppercase">
                            Scent Architecture
                        </h3>
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-base font-artistic-primary capitalize  leading-relaxed">
                            <div className="border-l-2 border-beige-light pl-3">
                                <span className="block font-bold text-beige-light uppercase text-[13px] tracking-wider">
                                    Top Notes
                                </span>
                                <p className="text-secondary-black text-[17px]  mt-0.5">
                                    {perfumeDetails?.notes?.top}
                                </p>
                            </div>
                            <div className="border-l-2 border-beige-dark pl-3">
                                <span className="block font-bold text-beige-dark uppercase text-[13px] tracking-wider">
                                    Heart Notes
                                </span>
                                <p className="text-secondary-black text-[17px]  mt-0.5">
                                    {perfumeDetails?.notes?.heart}
                                </p>
                            </div>
                            <div className="border-l-2 border-beige-accent pl-3">
                                <span className="block font-bold text-beige-accent uppercase text-[13px] tracking-wider">
                                    Base Notes
                                </span>
                                <p className="text-secondary-black text-[17px]  mt-0.5">
                                    {perfumeDetails?.notes?.base}
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col sm:flex-row items-stretch gap-4 pt-4 border-t-2 border-beige-light/60">
                        <div className="flex items-center justify-between border border-beige-light bg-primary-white rounded-xl px-4 py-3 sm:w-32">
                            <button
                                ...
                                className="text-beige-dark hover:text-primary-black text-xs transition-colors cursor-pointer"
                            >
                                <FaMinus />
                            </button>
                            <span className="font-bold text-sm text-primary-black font-secondary">
                                {quantity}
                            </span>
                            <button
                        ...
                        <button
                            ...
                            className={`flex-1 flex items-center justify-center gap-2 text-primary-white font-secondary text-sm font-bold py-4 px-6 rounded-xl transition-colors shadow-md ${
                                perfumeDetails?.inStock <= 0
                        ...
                        <button
                            onClick={(e) => {
                                ...
                                e.stopPropagation();
                                isWishlisted
                                    ? removeFromWishlistHandler(perfumeDetails?._id)
                                    : addToWishlistHandler(perfumeDetails?._id);
                            }}
                        ...
                        >
                            <FaHeart
                                className={isWishlisted ? "scale-110 transition-transform" : ""}
                            />
                        </button>
                    </div>
                    ...
                    <div className="pt-6 grid grid-cols-1 sm:grid-cols-3 gap-4 border-t-2 border-beige-light/60 font-secondary text-xs text-secondary-black">
                        <div className="flex items-center gap-2.5">
                            <FaShieldAlt className="text-green-dark" />{" "}
                            <span>100% Authentic Sourcing</span>
                        </div>
                        <div className="flex items-center gap-2.5">
                            <FaBarcode className="text-beige-accent" />{" "}
                            <span>Intact Original Batch Codes</span>
                        </div>
                        <div className="flex items-center gap-2.5">
                            <FaBoxOpen className="text-beige-dark" />{" "}
                            ...
                            <div className="space-y-1">
                                ...
                                <p className="font-secondary text-xs text-secondary-black leading-relaxed">
                                    ...
                                    <span className="font-semibold text-primary-black">
                                        {perfumeDetails?.brand} {perfumeDetails?.name}
                                    </span>
                                    ? This action removes it permanently from your digital
