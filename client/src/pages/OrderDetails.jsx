      ...
      if (loading) {
            return (
                  <div className="min-h-screen bg-secondary-white text-primary-black font-primary flex flex-col justify-center items-center">
                        ...
                        <p className="text-xs tracking-wider uppercase text-secondary-black font-secondary">Retrieving Ledger Record...</p>
                  </div>
            );
      }
      ...
      if (error || !order) {
            return (
                  <div className="min-h-screen bg-secondary-white text-primary-black font-primary flex flex-col justify-center items-center space-y-4">
                        <p className="text-sm font-secondary text-secondary-black">{error || "Order data matching this identifier was not found."}</p>
                        <Link to="/orders" className="text-xs font-semibold text-green-dark flex items-center gap-1.5 hover:underline underline-offset-4">
                                                ...
                                                return (
                                                      <div key={item._id} className="p-5 flex gap-4 items-center group">
                                                            ...
                                                            <div className="grow space-y-0.5">
                                                                  ...
                                                                  <p className="font-secondary text-xs text-secondary-black">
                                                                        Size: {item.size || item.product?.size || 'N/A'} &bull; Qty: {item.quantity}
                                                                  </p>
                                                            </div>
                                          ...
                                          <div className="space-y-1.5">
                                                ...
                                                {order.shippingAddress && (
                                                      <div className="text-secondary-black leading-relaxed capitalize">
                                                            <p className="font-semibold text-primary-black">{order.shippingAddress.name}</p>
                                                            <p>{order.shippingAddress.address}</p>
                                                            <p>{order.shippingAddress.city}, {order.shippingAddress.state} - {order.shippingAddress.pincode}</p>
                                                            <p className="uppercase">{order.shippingAddress.country}</p>
                                                            {order.shippingAddress.phone && <p className="normal-case mt-1 text-beige-dark">Phone: {order.shippingAddress.phone}</p>}
                                                      </div>
                                                )}
                                          </div>
