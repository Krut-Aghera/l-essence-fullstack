...
import { passwordResetTemplate, sendEmail } from "../utils/email.handler.js";
...
const userLogin = asyncHandler(async (req, res) => {
    ...
    if (!identifier || !password) {
        throw new ApiError(400, "Please provide both email/phone and password", [
            "emailOrPhone",
            "password",
        ]);
    }
...
const forgotPassword = asyncHandler(async (req, res) => {
    ...
    if (!isMailSent.success) {
        throw new ApiError(
            500,
            "Password reset failed due to email not being sent to your email, Please try again later."
        );
    }
...
const resetPassword = asyncHandler(async (req, res) => {
    ...
    if (!user) {
        throw new ApiError(
            404,
            "Invalid or expired token, try again with a new password reset request"
        );
    }
...
const accessTokenRefresh = asyncHandler(async (req, res) => {
    ...
    const { accessToken: generatedAccessToken, refreshToken: generatedRefreshToken } =
    ...
    res.status(200)
        ...
        .json(
            new ApiResponse(200, "Refreshing access token successful", {
                user: {
                    _id: user._id,
                    name: user.name,
                    email: user.email,
...
const changePassword = asyncHandler(async (req, res) => {
    ...
    res.status(200)
        ...
        .json(
            new ApiResponse(
                200,
                "Password changed successfully. Login with new password to continue."
            )
        );
});
