...
const calculateCartPricing = (cart) => {
    ...
    const subtotal = items.reduce((acc, item) => acc + item.perfume.price * item.quantity, 0);

    const shipping = subtotal > 300 || subtotal === 0 ? 0 : 15;

    const discountPercentage = cart?.appliedCoupon?.discountPercentage || 0;

    const discountAmount = ((subtotal + shipping) * discountPercentage) / 100;
